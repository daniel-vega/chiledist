...
- `chiledist/engines/metrics.py` (post-B1; antes `split_metrics.py`) provee: `count_split_units`, `split_severity_index`, `split_unit_summary`, `small_fragment_count`, `plan_split_metrics`, **y `pop_afectada_pct`** (ver más abajo).
- `compare_ensembles()` y `scenario_delta()` (`chiledist/inference/comparison.py`, post-B1; antes `scenario_comparison/compare.py`) permiten comparar medianas y percentiles entre escenarios.
- **✓ Implementado — `pop_afectada_pct(assignment, gdf, unit_col, id_col, pop_col)`** (`chiledist/engines/metrics.py:623`): fracción de la población total que reside en comunas partidas, exactamente la métrica que el borrador original pedía como "lo que falta completamente". Se calcula dentro de `plan_split_metrics()` (`chiledist/engines/metrics.py:674`, campo `pop_afectada_pct`) y se exporta en `chiledist/__init__.py`.
...
- **✓ Implementado — barrido de `split_penalty`**: `chiledist/inference/pareto_sweep/` (post-B1; antes `pareto_sweep/`) (`sweep_split_penalty`, `build_tradeoff_frontier`, `detect_knee_point`, `summarize_tradeoff`) y el script `scripts/pareto_sweep.py` corren automáticamente N configuraciones de `split_penalty` y consolidan sus distribuciones. Ya no es un experimento manual.
- **✓ Implementado — `weighted_population_balance()`** (`chiledist/evaluation/district_malapportionment.py:83`): balance poblacional ponderado por magnitud (distinto del uniforme de `population_balance()`), usado en la comparación balance uniforme vs ponderado de H1 (ver `SCIENTIFIC_HYPOTHESES.md`).
...
- **Tasa de intercambio explícita como función**: sigue sin existir un `costo_restriccion()` (o equivalente) que calcule `(median_max_dev_legal − median_max_dev_apc_free) / median_max_dev_apc_free` como una función reutilizable. `scenario_delta()` (`chiledist/inference/comparison.py`, post-B1) calcula deltas absolutos respecto a un baseline, pero no esta razón normalizada específica. Es un cálculo derivado trivial de `compare_ensembles()`, no una brecha de infraestructura.
...
- `chiledist/domain/equivalence.py` (post-B1; antes `chiledist/equivalence.py`) (tabla USA↔Chile): referencia conceptual, no analítica. No aporta a H1.
...
- `plot_tradeoff_frontier()` (`chiledist/inference/plots.py`, post-B1; antes `scenario_comparison/plots.py`) genera un scatter 2D con la frontera.
...
- **✓ Implementado — barrido de `split_penalty` para frontera continua**: `chiledist/inference/pareto_sweep/frontier.py::sweep_split_penalty` + `build_tradeoff_frontier` (post-B1; antes `chiledist/pareto_sweep/frontier.py`) corren automáticamente el rango de valores y consolidan el dataset. `scripts/pareto_sweep.py` es el entrypoint CLI. Ya no hace falta correr el pipeline manualmente valor por valor.
- **✓ Implementado — posicionamiento del mapa vigente**: `position_plan_vigente()` (`chiledist/inference/sensitivity.py:96`, post-B1; antes `scenario_comparison/sensitivity.py`) calcula el mismo vector de métricas (max_dev_pob_pct, pp_promedio, cut_edges, métricas de split) para el mapa vigente y lo posiciona junto a los ensembles, respondiendo directamente si el vigente es Pareto-dominado.
- **✓ Implementado — caracterización de la forma de la frontera**: `detect_knee_point()` (`chiledist/inference/pareto_sweep/frontier.py:331`) detecta el punto de codo / retornos decrecientes sobre la frontera trazada por el barrido.
- **✓ Implementado — bandas de confianza en la frontera**: `build_tradeoff_frontier()` acepta bootstrap y calcula `bootstrap_bands` (`chiledist/inference/pareto_sweep/frontier.py:93-181, 306-315`) — cada punto de la frontera lleva su banda de incertidumbre, no solo la mediana puntual.
...
- **✓ Implementado — `personas_por_escano(pop_by_district, magnitudes)`** (`chiledist/evaluation/district_malapportionment.py:16`, post-B1; antes `chiledist/electoral/district_malapportionment.py`): la métrica fundamental de H3, personas representadas por escaño en cada circunscripción.
- **✓ Implementado — `peso_relativo_del_voto(pop_by_district, magnitudes)`** (`chiledist/evaluation/district_malapportionment.py:43`): razón `personas_por_escano_i / media_nacional`, la medida estándar de malapportionment comparado (Samuels & Snyder 2001).
- **✓ Implementado — comparación contrafactual de magnitudes**: `comparar_magnitudes()` (`chiledist/engines/allocation/magnitudes.py:165`, post-B1; antes `chiledist/electoral/magnitudes.py`) compara las magnitudes vigentes (`MAGNITUDES_LEGALES_LEY20840`) contra las que resultarían del método Hamilton bajo una población actualizada — el experimento contrafactual central de H3. Ahora existe además `datos/asignacion_vigente.json` (346 comunas → 28 distritos, ver sección "Datos externos" del README) como el mapa vigente real con el que alimentar esta comparación desde `scripts/malapportionment.py --assignment-path`.
- **✓ Implementado — Índice de Gini de representación**: `gini_personas_por_escano()` (`chiledist/evaluation/malapportionment/indices.py:144`, post-B1; antes `chiledist/malapportionment/indices.py`), junto con un módulo completo (`samuels_snyder_index`, `max_min_representation_ratio`, `malapportionment_summary`, `international_comparison` con benchmarks internacionales) que excede lo que pedía el borrador original.
- **✓ Implementado — `assign_seat_magnitudes_dhondt()`** (`chiledist/engines/allocation/magnitudes.py:102`): asignación de magnitudes por el método D'Hondt (Art. 121 Ley 18.700, el método legalmente correcto, distinto del Hamilton acotado de `assign_seat_magnitudes()`). Validado contra la Resolución O 129/2026: 26/28 coincidencias exactas (ver `SCIENTIFIC_HYPOTHESES.md § H3`).
- **✓ Implementado — `MAGNITUDES_CENSO2024_2026`** (`chiledist/rules/electoral_rules.py:69`): tabla de magnitudes vigentes desde el 18-ABR-2026 (Resolución O 129/2026, basada en Censo 2024), disponible junto a `MAGNITUDES_LEGALES_LEY20840` (régimen 2021-2025) y seleccionable en `scripts/malapportionment.py --magnitudes {ley20840,censo2026}`.
...
- **✓ Implementado — `compare_sensitivity(ensembles, metric_cols)`** (`chiledist/inference/sensitivity.py:216`, post-B1; antes `chiledist/scenario_comparison/sensitivity.py:213`): tabla de comparación por pares entre variantes metodológicas (KS test, diferencia de medianas, tamaño de efecto) — exactamente el framework de sensibilidad que el borrador marcaba como ausente.
- **✓ Implementado — `ranking_concordance(scores_a, scores_b)`** (`chiledist/inference/sensitivity.py:320`, post-B1; antes `chiledist/scenario_comparison/sensitivity.py:317`): Kendall τ + Spearman ρ + lista de pares discordantes entre rankings bajo distintas condiciones metodológicas.
...
| **H1** | `weighted_population_balance()` (balance ponderado por magnitud) | ✅ Completo | — | — |
...
| **H3** | `assign_seat_magnitudes_dhondt()` (D'Hondt, Art. 121) — validado 26/28 vs Resolución O 129/2026 | ✅ Completo | — | — |
| **H3** | `MAGNITUDES_CENSO2024_2026` (Resolución O 129/2026) | ✅ Completo | — | — |
...
| **H4** | `validate_election()` + `ValidationReport` — validado **28/28 distritos, `EXACT_REPRODUCTION`** vs TRICEL 2025 (`dhondt_binivel_cl()`, agosto 2026); 25/28 `PARTIAL` con `dhondt_binivel()` genérica; 96/96 combinaciones vs SERVEL 2025 (`validar_dhondt.py`, sin pasar por `validate_election()`) | ✅ Completo | — | — |
...
- **`pop_afectada_pct(assignment, gdf, unit_col, id_col, pop_col)`** (`chiledist/engines/metrics.py:623`, post-B1) — fracción de la población total que reside en comunas partidas. Distinción respecto a `share_comunas_partidas` (nº comunas partidas / total comunas, ya existía antes): `pop_afectada_pct` pondera por población, no por conteo de comunas, que es el número interpretable en discusión legislativa.
...
Es un cálculo derivado de `compare_ensembles()` de una sola línea; su ausencia es una conveniencia faltante, no una brecha de infraestructura. Implementarla como `chiledist.inference.costo_restriccion(df_comp, baseline, target)` (post-B1; namespace antes `chiledist.scenario_comparison`) sería el ajuste mínimo restante para H1.
...
- **Barrido de `split_penalty`** → `chiledist/inference/pareto_sweep/frontier.py::sweep_split_penalty`, `build_tradeoff_frontier` (post-B1), orquestado por `scripts/pareto_sweep.py`.
- **Posicionamiento del mapa vigente** → `chiledist/inference/sensitivity.py::position_plan_vigente` (post-B1).
- **Forma de la frontera** → `chiledist/inference/pareto_sweep/frontier.py::detect_knee_point` (post-B1).
...
- **`personas_por_escano(pop_by_district, magnitudes)`** → `chiledist/evaluation/district_malapportionment.py:16` (post-B1; antes `chiledist/electoral/district_malapportionment.py`).
- **`peso_relativo_del_voto(pop_by_district, magnitudes)`** → `chiledist/evaluation/district_malapportionment.py:43`.
- **Comparación contrafactual de magnitudes** → `chiledist/engines/allocation/magnitudes.py::comparar_magnitudes` (post-B1; antes `chiledist/electoral/magnitudes.py`), alimentable con población real (Censo 2024) y con el mapa vigente real (`datos/asignacion_vigente.json`, ver README § "Datos externos").
- **Índice de Gini de representación** → `chiledist/evaluation/malapportionment/indices.py::gini_personas_por_escano` (post-B1; antes `chiledist/malapportionment/indices.py`), más un módulo `chiledist.evaluation.malapportionment` completo (Samuels-Snyder, Loosemore-Hanby aplicado a escaños, comparación internacional).
- **`assign_seat_magnitudes_dhondt()`** → `chiledist/engines/allocation/magnitudes.py:102` — D'Hondt Art. 121, validado 26/28 vs Resolución O 129/2026.
- **`MAGNITUDES_CENSO2024_2026`** → `chiledist/rules/electoral_rules.py:69`.
...
**D'Hondt binivel (pactos → partidos)** — antes la brecha más crítica del módulo electoral — está implementado en `chiledist/engines/allocation/dhondt.py::dhondt_binivel` / `run_electoral_plan_binivel` (post-B1; antes `chiledist/electoral/dhondt.py`): D'Hondt entre pactos sobre votos totales de lista, luego asignación intra-pacto por mayor votación individual. Validado 96/96 combinaciones (distrito, pacto) vs SERVEL 2025 (`validar_dhondt.py`). **`dhondt_binivel_cl()`** (misma ruta, agosto 2026, función separada — `dhondt_binivel()` sin cambios): agrega tope de candidatos disponibles por partido en el Nivel 2, causa raíz verificada de las únicas discrepancias contra TRICEL — con ella, **28/28 distritos, `EXACT_REPRODUCTION`**; con `dhondt_binivel()` genérica, 25/28 `PARTIAL` (`validar_tricel.py`) — ver `VALIDATION_REPORT.md` §9-10. `chiledist/inference/electoral_ensemble/core.py::run_electoral_ensemble` (post-B1; antes `chiledist/electoral_ensemble/`) lo integra sobre ensembles completos cuando se provee `pacto_map` — sigue usando `dhondt_binivel()` genérica (no `_cl`): no hay candidatos reales que consultar para distritos sintéticos de un ensemble (ver `VALIDATION_REPORT.md` §10).

**`margen_ultimo_escano(votos_district, magnitud)`** → `chiledist/evaluation/district_malapportionment.py:204` (post-B1).

**`umbral_efectivo(magnitud)`** → `chiledist/evaluation/district_malapportionment.py:175` (post-B1), fórmula `1/(M+1)`.

**`seat_bonus(vote_shares, seat_shares)`** → `chiledist/evaluation/proportionality.py:125` (post-B1; antes `chiledist/electoral/proportionality.py`).
...
**`validate_election()` + `ValidationReport`** → `chiledist/validation/__init__.py` — valida el resultado calculado contra las proclamaciones TRICEL; es el mecanismo detrás de la validación TRICEL — 28/28, `EXACT_REPRODUCTION`, con `candidatos_por_partido`/`dhondt_binivel_cl()` (agosto 2026); 25/28 `PARTIAL` sin ese parámetro — distinta del 96/96 vs SERVEL (`validar_dhondt.py`, no usa `validate_election()`).
...
**Comparación de distribuciones entre variantes metodológicas** → `chiledist/inference/sensitivity.py::compare_sensitivity(ensembles, metric_cols)` (post-B1): KS test, p-value y diferencia de medianas por métrica, entre pares de ensembles.

**Concordancia de ranking entre variantes** → `chiledist/inference/sensitivity.py::ranking_concordance(scores_a, scores_b)` (post-B1): Kendall τ, Spearman ρ, pares discordantes.
...
- Preflight de factibilidad poblacional (`chiledist/rules/feasibility.py::check_population_feasibility`, post-B1; antes `chiledist/feasibility.py`) — separa inviabilidad matemática probada (`status: "infeasible_population"`) de fallo de búsqueda de inicialización (`status: "sin_particion"`), evidencia directamente relevante para argumentar robustez o sus límites.
- Marcado de comparaciones incompletas (`chiledist/domain/ensemble_store.py::assess_comparison_completeness`, post-B1; antes `chiledist/scenario_comparison/compare.py`) — evita reportar un ranking parcial (ej. sin el baseline `legal_comunas`) como si fuera una comparación H1 completa.

No queda ningún ajuste de infraestructura pendiente identificado para H5. Pendiente de **ejecución** (no de infraestructura): correr `run_chains.py` con ≥4 semillas independientes sobre los ensembles reales de H1 — ver Prioridad § 9.
...
- *Personas por escaño por circunscripción* con datos del Censo 2024. Tabla de 28 filas. **✓ Computable** con `personas_por_escano()` (`chiledist/evaluation/district_malapportionment.py:16`, post-B1), usando `datos/asignacion_vigente.json` como mapa comuna→distrito y Censo 2024 como fuente de población.
...
- *Cambio en magnitudes requeridas con Censo 2024*: tabla de 28 distritos mostrando escaños actuales vs escaños bajo metodología actualizada. **✓ Computable** con `comparar_magnitudes()` (`chiledist/engines/allocation/magnitudes.py:165`, post-B1) o directamente con `assign_seat_magnitudes_dhondt()` + `MAGNITUDES_CENSO2024_2026` (método legal, validado 26/28 vs Resolución O 129/2026).
...
**RESOLVED desde la última revisión (agosto 2026, post-refactorización B1):** `assign_seat_magnitudes_dhondt()` (validado 26/28 vs Resolución O 129/2026), `MAGNITUDES_CENSO2024_2026`, `weighted_population_balance()`, la capa de datos SERVEL/TRICEL (`import_candidates()`, `import_proclamations()`, `import_votes()`, `import_candidate_counts()`) y `validate_election()` + `ValidationReport` (validado **28/28 distritos, `EXACT_REPRODUCTION`, vs TRICEL 2025** con `dhondt_binivel_cl()`, agosto 2026 — 25/28 `PARTIAL` con `dhondt_binivel()` genérica, preservada sin cambios; 96/96 combinaciones vs SERVEL 2025 por separado vía `validar_dhondt.py`, sin cambios — ver `VALIDATION_REPORT.md` §9-10). H1, H2, H3, H4, H8 y H9 corrieron con datos reales (ver `SCIENTIFIC_HYPOTHESES.md § Estado consolidado`).
...
| Ejecución de `run_chains.py` con ≥4 semillas independientes | H5 | Alto: sin esto H5 no tiene resultado con datos reales | Medio: robustez metodológica para defender el diseño ante objeciones | Baja (infraestructura ya existe) |
| `apc_strict` vía SMC (descomposición causal completa) | H1 | Medio: separaría efecto resolución de efecto restricción | Bajo: la comparación directa legal/apc_free/apc_soft ya cierra H1 | Alta (ReCom no converge; requiere R + paquete `redist`) |
| Ensemble nacional de 28 distritos (`--regiones nacional_comunal --n-distritos 28`) | H4 (B3), H6, H7 | Alto: bloquea distribución de Gallagher/seat_bonus sobre planes alternativos a escala nacional | Alto: "¿el mapa vigente es atípico entre las alternativas posibles?" solo respondible con esto | Alta (corrida nacional, no regional) |
| `fair_share_matrix()` para el mapa vigente (plan único) | H6, H7 | Medio: función ya existe, falta ejecutarla sobre el plan vigente | Medio: cuantifica distancia al ideal biproporcional | Baja (función implementada, falta ejecución) |
...
| `chiledist/domain/equivalence.py` (post-B1; antes `chiledist/equivalence.py`) (tabla USA↔Chile, `print_equivalence`, `describe_hierarchy`) | Referencia conceptual. No tiene uso en ningún pipeline analítico de H1–H5. |
| `chiledist/viz.py` (mayoría de funciones) | Útil para comunicación; no para producir resultados científicos. `plot_tradeoff_frontier` y `plot_boxplots_comparativos` (usadas en H1–H2) viven en `chiledist/inference/plots.py` (post-B1; antes `chiledist/scenario_comparison/plots.py`), no en `viz.py` — corrección respecto al borrador original, que las atribuía a `viz.py`. El resto de `viz.py` no aporta. |
...
**Última actualización: agosto 2026 — post-refactorización B1 y validación electoral TRICEL 2025.**
